...
while True:
    ...
    ##############################################################################################################################################################
            ...
            if flag == "svm":
                classifier = SVC_model.predict([earPredict])
    ##############################################################################################################################################################
    # below is if you want to use the trained idea
            with open('confusionMatrix_all_threshold_0.20.csv', 'a', newline='') as file:
                ...
                all_Frames.writerow([FrameCounter, "0"]) # open - setting all open rn , we merge - repalce this with the blink to get teh actual data
    ...
    ##
            ...
            if flag != "svm":
                ...
                cv2.putText(frame, tempString.format(TOTAL), (10, 50),
                            cv2.FONT_HERSHEY_SIMPLEX, 0.7, (255, 255, 255), 2)
